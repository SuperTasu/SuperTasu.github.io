...
onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
        // ログイン状態
        authBtn.querySelector('i').classList.add('hidden'); // Hide icon
        ...
        // ログアウト状態
        authBtn.querySelector('i').classList.remove('hidden'); // Show icon
        userAvatar.style.display = 'none';
        userAvatar.src = "";
        authBtn.title = "Googleログイン";
        
        // LocalStorageのデータを使用（何もしない、またはデフォルトに戻す）
        // ここでは、ユーザーが切り替わった場合を考慮し、再読込などを促すか、
        // 単純に現在のLocalStorageの状態を維持します。
    }
});
...
async function loadFromCloud() {
    ...
    try {
        ...
        if (docSnap.exists()) {
            ...
            // Background
            if (data.background) {
                saveBackground(data.background, false); // false = don't push back to cloud
            }
...
function switchSubTab(targetId) {
    ...
    // SubTabはlocalStorageに保存されていますが、Firestore同期対象にはしていません（要件外と判断）。
    // 必要であればここでも saveToCloud() を呼び出します。
...
window.saveBackground = saveBackground; // Option画面から呼ばれる可能性があるため
window.saveTextColor = saveTextColor; // Option画面から呼ばれる可能性があるため
